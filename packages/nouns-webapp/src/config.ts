...
export const createNetworkHttpUrl = (network: string): string => {
  ...
  return custom || `https://eth-${network}.g.alchemy.com/v2/${INFURA_PROJECT_ID}`;
};

export const createNetworkWsUrl = (network: string): string => {
  const custom = process.env[`REACT_APP_${network.toUpperCase()}_WSRPC`];
  return custom || `wss://eth-${network}.g.alchemy.com/v2/${INFURA_PROJECT_ID}`;
};
...
const app: Record<SupportedChains, AppConfig> = {
  [ChainId.Rinkeby]: {
    ...
    subgraphApiUri: 'https://api.studio.thegraph.com/query/35052/nounsters-subgraph-goerli/v0.0.5',
